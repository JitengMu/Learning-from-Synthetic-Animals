...
best_acc = 0
best_auc = 0
best_mean_error = 10.0
best_epoch_acc = 0
best_epoch_auc = 0
best_epoch_mean_error = 0
idx = []
...
def main(args):
    global best_acc
    global best_auc
    global best_mean_error
    global best_epoch_acc
    global best_epoch_auc
    global best_epoch_mean_error
    global idx
    ...
    if args.resume:
        ...
        logger.set_names(['Epoch', 'LR', 'Train Loss', 'Val Loss',
                          'Train Acc', 'Val Acc', 'Val Auc', 'Val Mean Error'])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        valid_loss, valid_acc, predictions, valid_auc, valid_mean_error = validate(val_loader, model, criterion,
                                                  args.debug, args.flip, args.test_batch, njoints)

        # append logger file
        logger.append([epoch + 1, lr, train_loss, valid_loss, train_acc, valid_acc, valid_auc, valid_mean_error])
        ...
        if valid_mean_error<best_mean_error:
            best_epoch_mean_error = epoch
        best_acc = max(valid_acc, best_acc)
        best_auc = max(valid_auc, best_auc)
        best_mean_error = min(valid_mean_error, best_mean_error)
        save_checkpoint({
            'epoch': epoch + 1,
            'arch': args.arch,
    ...
    print(best_epoch_acc, best_acc, best_epoch_auc, best_auc, best_epoch_mean_error, best_mean_error)
    logger.close()
    #logger.plot(['Train Acc', 'Val Acc'])
    savefig(os.path.join(args.checkpoint, 'log.eps'))

def train(train_loader, model, criterion, optimizer, debug=False, flip=True, train_batch=6, epoch=0, njoints=68):
    ...
    for i, (input, target, meta) in enumerate(train_loader):
        ...
        interocular_dists[:, i * train_batch:(i + 1) * train_batch] = batch_interocular_dists
    ...
    mean_error = torch.mean(interocular_dists_pickup[interocular_dists_pickup!=-1])
    auc = calc_metrics(interocular_dists, idx) # this is auc of predicted maps and target.
    #print("=> Mean Error: {:.8f}, AUC@0.08: {:.8f} based on maps".format(mean_error, auc))
...
def validate(val_loader, model, criterion, debug=False, flip=True, test_batch=6, njoints=68):
    ...
    with torch.no_grad():
        for i, (input, target, meta) in enumerate(val_loader):
            ...
            interocular_dists[:, i * test_batch:(i + 1) * test_batch] = batch_interocular_dists
