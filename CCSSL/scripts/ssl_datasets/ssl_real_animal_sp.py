...
def train_test_split1(idxs, img_list, animal):
    val_length = len(idxs)//5
    valid_idxs = np.random.choice(idxs, val_length, replace=False)
    train_idxs = []
    for i in idxs:
        if i not in valid_idxs:
            train_idxs.append(i)

    if isfile('../data/real_animal/' +animal+ '/train_idxs.npy'):
        train_idxs = np.load('../data/real_animal/' +animal+ '/train_idxs.npy')
        valid_idxs = np.load('../data/real_animal/' +animal+ '/valid_idxs.npy')
    else:
        np.save('../data/real_animal/' +animal+ '/train_idxs.npy', train_idxs)
        np.save('../data/real_animal/' +animal+ '/valid_idxs.npy', valid_idxs)

    print('split-random number of training images: ', len(train_idxs))
    print('split-random number of testing images: ', len(valid_idxs))
