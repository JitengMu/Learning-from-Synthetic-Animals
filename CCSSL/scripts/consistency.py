...
def prediction_check(previous_inp, previous_kpts, inp, model, dataset, num_transform=5, num_kpts=18, lambda_decay=0.9):
    """
    Input:
        Image: 3x256x256
    Output:
    ...
    for i in range(num_transform):
        ...
        #if is_flip:
        #    img = torch.from_numpy(fliplr(img.cpu().numpy())).float().to(device)
        img = crop(img, c, s, [256, 256], rot)
...
def kpts_to_heatmap(generated_kpts):
    ...
    for j in range(num_kpts):
        ...
        target[j], vis = draw_labelmap(target[j], tpts[j]-1, 1, type='Gaussian')
