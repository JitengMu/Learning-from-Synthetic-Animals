...
class Synthetic_Animal_SP(data.Dataset):
    def __init__(self, is_train = True, **kwargs):
        ...
        if self.animal=='horse':
            ...
            self.idxs = np.array([2046,1944,1267,1875,1900,1868,1894,687,173,1829,1422,821,624,580,622,575,1370,716])
            self.idxs_mask = np.zeros(3299)
        elif self.animal=='elephant':
            self.idxs = np.array([1980,2051,1734,2122,2155,2070,2166,681,923,1442,1041,1528,78,599,25,595,171,570, 1147,1211,1208,1226,1320,1275,1265])
            self.idxs_mask = np.zeros(3299)
        elif self.animal=='hound':
            self.idxs = np.array([2028,2580,912,878,977,1541,1734,480,799,1575,1446,602,755,673,780,1580,466,631])
            self.idxs_mask = np.zeros(3299)
        else:
